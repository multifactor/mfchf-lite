...
async function verify (password, otp, mfchf) {
  ...
  const target = mod(offset + otp, 10 ** 6)
  const hash = await argon2.hash({ pass: password + target, salt, time: 100, mem: 4096, type: argon2.ArgonType.Argon2id })
  const sha = crypto.createHash('sha256').update(hash.hash).digest('base64')
  ...
  const code = parseInt(speakeasy.hotp({ secret: secret.toString('hex'), encoding: 'hex', counter: ctr, algorithm: 'sha1', digits: 6 }))
  const next = mod(target - code, 10 ** 6)

  const out = 'mfchf-argon2id-hotp6#' + ctr + ',' + next + ',' + pad.toString('base64') + '#' + sha + '#' + salt.toString('base64')
